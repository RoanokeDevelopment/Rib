...
class ConfiguredGUI(
    ...
    companion object {
        fun fromJson(json: JsonObject, items: MutableMap<String, ItemBuilder>): ConfiguredGUI? {
            ...
            val guiList: List<String> = json["gui"]?.jsonArray?.map { it.jsonPrimitive.content } ?: return null
    ...
    fun getGuiElementFromKey(key: String): GuiElementBuilder {
        return GuiElementBuilder.from(
            keys.getOrDefault(key,
                ItemBuilder("minecraft:dirt")
        ).build())
    }
